...
def balanced(m):
    ...
    flag3 = total_weight(end(left(m))) * length(left(m)) == total_weight(end(right(m))) * length(right(m))
...
def preorder(t):
    ...
    "*** YOUR CODE HERE ***"
    list = []
    def preorder_process(t):
        list.append(label(t))
        for branch in branches(t):
            preorder_process(branch)
    preorder_process(t)
    return list
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y) * upper_bound(y) > 0, "division by zero"
    reciprocal_y = interval(1/upper_bound(y), 1/lower_bound(y))
    return mul_interval(x, reciprocal_y)


def multiple_references_explanation():
    return """The multiple reference problem occurs when an expression refers to the same uncertain value multiple times, 
    ...
    more accurate calculations. Therefore, Eva Lu Ator is correct that par2 is a better program."""
...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x) <= extreme_point <= upper_bound(x):
        ...
        return interval(min(y1, y3), max(y1, y3))
