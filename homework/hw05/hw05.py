...
def is_bst(t):
    ...
    if len(t.branches) > 2:
        return False
    ans = []
    def is_bst_process(t):
        if Tree.is_leaf(t):
            ans.append(t.label)
            return
        if len(t.branches) > 1:
            is_bst_process(t.branches[0])
            ans.append(t.label)
            is_bst_process(t.branches[1])
        else:
...
def preorder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def preorder_process(t, ans=[]):
        ans.append(t.label)
        for b in t.branches:
            preorder_process(b, ans)
    ans = []
    preorder_process(t, ans)
    return ans
...
def path_yielder(t, value):
    ...
    #         yield new_lst
    #     for b in t.branches:
    #         yield from find_path(b, value, new_lst[:])
    # yield from find_path(t, value, [])
