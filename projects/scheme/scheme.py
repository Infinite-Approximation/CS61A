...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        opearnds = rest.map(lambda opearand: scheme_eval(opearand, env))
        return scheme_apply(operator, opearnds, env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(name, do_lambda_form(expressions, env))
        return name
        # END PROBLEM 9
    else:
...
def do_mu_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    body = expressions.rest
    return MuProcedure(formals, body)  # 返回MuProcedure实例
    # END PROBLEM 18
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        """在环境 ENV 中评估 Scheme 表达式 EXPR。如果 TAIL 为 True，
        则返回一个 Thunk 实例，表示需要进一步评估的表达式。
        """
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr):
            # 如果在尾调用上下文中，并且表达式不是原子表达式或自我评估的表达式
