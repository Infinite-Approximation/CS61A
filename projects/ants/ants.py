...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            assert self.can_contain(place.ant) or place.ant.can_contain(self), 'Neither ant can contain the other'
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        range = 0
        while cur_place and cur_place.name != 'Hive':
            if cur_place.bees and (self.min_range <= range <= self.max_range or
                                   self.min_range == 5 and range == 0):  # 对于LongThrower来说太近了就需要攻击
                return rANTdom_else_none(cur_place.bees)  # REPLACE THIS LINE
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        Ant.reduce_armor(self, amount)  # 自己受到amount点伤害
        # 让火蚁所在的place里的蜜蜂都受到amount点伤害，如果火蚁死了就再加self.damage伤害
        extra_amount = self.damage if self.armor <= 0 else 0
        for bee in bees:
            Bee.reduce_armor(bee, amount + extra_amount)
        # END Problem 5
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        ...
        if not QueenAnt.is_true_queen_exit:
            self.true_queen = True
            QueenAnt.is_true_queen_exit = True
        else:
    ...
    def action(self, gamestate):
        ...
        ThrowerAnt.action(self, gamestate)  # 作为一个ThrowAnt，需要攻击Bees
        cur_place = self.place.exit if self.place else None
        while cur_place:
            cur_place_ant = cur_place.ant
            if cur_place_ant and not cur_place_ant.is_damage_double:
                cur_place_ant.damage = cur_place_ant.damage * 2
                cur_place_ant.is_damage_double = True
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        destination = self.place.exit if self.direction == 1 else self.place.entrance
        # END EC
        if self.blocked():
            self.sting(self.place.ant)
        elif self.armor > 0 and destination is not None and destination.name != 'Hive':
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        bees = self.place.bees[:]
        # 让NinjaAnt所在的place里的蜜蜂都受到amount点伤害
        for bee in bees:
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        bees = self.place.bees[:]
        # 让TankAnt所在的place里的蜜蜂都受到amount点伤害
        for bee in bees:
            Bee.reduce_armor(bee, self.damage)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        distance = 0
        flag = False # 第一次不能把火蚁加进去
        while cur_place != beehive:
            if cur_place.ant and flag:
