...
def roll_dice(num_rolls, dice=six_sided):
    ...
    count = 0
    is_one_occur = False
    while iter < num_rolls:
        dice_result = dice()
        if dice_result == 1:
            is_one_occur = True
        count += dice_result
        iter += 1
    return 1 if is_one_occur else count
    # END PROBLEM 1
...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        return False
    return math.gcd(player_score, opponent_score) >= 10
    # END PROBLEM 4a
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    # last_score: 指的是上一轮的得分
    # running_high: 指的是最大的得分增加量
    # 注释：announce_highest返回的是一个函数，接受(score0, score1)。
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if free_bacon_points >= cutoff or extra_turn(score + free_bacon_points, opponent_score):
        return 0
    return num_rolls  # Replace this statement
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    return  extra_turn_strategy(score, opponent_score, 8, 4) # Replace this statement
    # END PROBLEM 12
